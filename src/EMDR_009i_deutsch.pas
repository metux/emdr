...
Var epo,muo  : Double;  {Naturkonstanten}
    ...
    SpIx,SpIy,SpIz    : Array [1..SpNmax] of Double; {Stützpunkte der Polygonzüge der Input-Spule, kartesische Koordinaten}
    SpTx,SpTy,SpTz    : Array [1..SpNmax] of Double; {Stützpunkte der Polygonzüge der Turbo-Spule, kartesische Koordinaten}
    ...
    FlIx,FlIy,FlIz    : Array [1..FlNmax] of Double; {Flächenelemente der Input-Spule, kartesische Koordinaten}
    FlTx,FlTy,FlTz    : Array [1..FlNmax] of Double; {Flächenelemente der Turbo-Spule, kartesische Koordinaten}
    ...
    PSIinput,PSIturbo   : Array[0..AnzPmax] of Double; {Magnetischer Fluß in den Spulen: Input-Spule und Turbo-Spule}
    ...
    PlotAnfang,PlotEnde,PlotStep : LongInt; {Zählung in "i": Anfang,Ende,Schrittweite des Daten-Exports nach Excel}
    ...
    CT          : Double;  {Farad}  {Kapazität des Kondensators, er mit in der Turbo-Spule in Reihe geschaltet}
    CI          : Double;  {Farad}  {Kapazität des Kondensators, er mit in der Input-Spule in Reihe geschaltet}
    ...
    QTmax,QImax,QpTmax,QpImax,QppTmax,QppImax,phipomax : Double; {Maximalwerte finden für Strom-, Drehzahl- und Spannungsangabe}
...
begin
  ...
  Writeln(fout,'Abstd= ',Abstd:5,'  {Nur Vorbereitung, nicht zum Lösen der Dgl.: Jeden wievielten Punkt soll ich plotten ins Excel?');
  Writeln(fout,'PlotAnfang = ',Round(PlotAnfang):10,' {Zum Lösen der Dgl.: Erster-Plot-Punkt: Anfang des Daten-Exports nach Excel');
  Writeln(fout,'PlotEnde   = ',Round(PlotEnde):10,' {Zum Lösen der Dgl.: Letzter-Plot-Punkt: Ende des Daten-Exports nach Excel');
  Writeln(fout,'PlotStep   = ',Round(PlotStep):10,' {Zum Lösen der Dgl.: Schrittweite des Daten-Exports nach Excel');
  Writeln(fout,' ');
  ...
  Writeln(fout,'CT = ',CT:14,'  {Farad}    {Kapazität des Kondensators, der mit in der Turbo-Spule (!) in Reihe geschaltet');
  Writeln(fout,'CI = ',CI:14,'  {Farad}    {Kapazität des Kondensators, der mit in der Input-Spule (!) in Reihe geschaltet');
  Writeln(fout,' ');
  ...
  Writeln(fout,' ');
  Writeln(fout,'Abgeleitete Parameter. Die Größen werden aus den obigen Parametern berechnet, es ist keine Eingabe möglich:');
  ...
  Writeln(fout,'fkI:=Sqrt(HoeheI*HoeheI+4/pi*2*yo*2*zo)/HoeheI = ',fkI:10:5,' Korrekturfaktor zur Induktivität der kurzen Input-Spule');
  Writeln(fout,'fkT:=Sqrt(HoeheT*HoeheT+4/pi*2*yo*2*zo)/HoeheT = ',fkT:10:5,' Korrekturfaktor zur Induktivität der kurzen Turbo-Spule');
  Writeln(fout,'LI:=muo*(2*yo+BreiteI)*(2*zo+BreiteI)*Ninput*Ninput/(HoeheI*fkI) = ',LI,' Induktivität Input-Spule');
  Writeln(fout,'LT:=muo*(2*yo+BreiteT)*(2*zo+Breitet)*Nturbo*Nturbo/(HoeheT*fkT) = ',LT,' Induktivität Turbo-Spule');
  ...
  Writeln(fout,'J:=Mmag/4*(MEro*MEro+4*MEyo*MEyo/3) = ',J,' Trägheitsmoment des Magneten bei Rotation um Querachse');
  Writeln(fout,' ');
...
begin
  ...
  begin   {gehe dann in z-Richtung zurück}
    SpIx[2*zo+2*yo+i+1]:=-xo*Spsw;  SpIy[2*zo+2*yo+i+1]:=yo*Spsw;  SpIz[2*zo+2*yo+i+1]:=(zo-i)*Spsw;     {Stützpunkt}
    SpTx[2*zo+2*yo+i+1]:=+xo*Spsw;  SpTy[2*zo+2*yo+i+1]:=yo*Spsw;  SpTz[2*zo+2*yo+i+1]:=(zo-i)*Spsw;     {Stützpunkt}
  ...
  begin   {und zu guter Letzt in y-Richtung wieder runter}
    SpIx[4*zo+2*yo+i+1]:=-xo*Spsw;  SpIy[4*zo+2*yo+i+1]:=(yo-i)*Spsw;      SpIz[4*zo+2*yo+i+1]:=-zo*Spsw; {Stützpunkt}
    SpTx[4*zo+2*yo+i+1]:=+xo*Spsw;  SpTy[4*zo+2*yo+i+1]:=(yo-i)*Spsw;      SpTz[4*zo+2*yo+i+1]:=-zo*Spsw; {Stützpunkt}
...
Var i : Integer; {Laufvariable}
    ...
    dMx,dMy,dMz : Double; {Drehmoment, das jedes Leiterschleifen-Element auf seinen gesamten Magneten ausübt.}
...
{ If i<=Pulsdauer then Umerk:=UAmpl;}          {falls gewünscht: Start-Impuls geben}
  ...
  begin
    ...
    begin Umerk:=UAmpl; end; {Spannung anlegen}                     {Alternativ könnte man ihn z.B. auch am Nulldurchgang festmachen.}
  end;
...
begin  {Eine kleine Schalt-Hysterese muß ich einbauen:}
  ...
  If (phipo/2/pi*60)<0.999999*phipZiel then merk:=cr*0.999999; {Wenn's zu langsam läuft => weniger Reibung}
...
{ Allgemeine technische Größen:}
  ...
  CT:=101.7E-6;  {150E-6;} {Farad}    {Kapazität des Kondensators, der mit in der Turbo-Spule (!) in Reihe geschaltet}
  CI:=100E-6;    {Farad}   {Kapazität des Kondensators, der mit in der Input-Spule (!) in Reihe geschaltet}
...
{ Abgeleitete Parameter. Die Größen werden aus den obigen Parametern berechnet, es ist keine Eingabe möglich:}
  ...
  fkI:=Sqrt(HoeheI*HoeheI+4/pi*2*yo*2*zo)/HoeheI; {Korrekturfaktor zur Induktivität der kurzen Inpuz-Spule nach *1 von S.16}
  ...
  LI:=muo*(2*yo+BreiteI)*(2*zo+BreiteI)*Ninput*Ninput/(HoeheI*fkI); {Geometrische Mittelung => Induktivität der Input-Spule}
  ...
  J:=Mmag/4*(MEro*MEro+4*MEyo*MEyo/3); {Trägheitsmoment des Magneten bei Rotation, siehe *2 von S.13 und Dubbel S.B-32}
...
{ Hier beginnt das Rechenprogramm.}
  ...
  Magfeld_Input_Berechnen(-0.00,0.01,0.01,1.0);  {drei kartesische Komponenten für Aufpunkt und Strom = 1.0 Ampere}
  Writeln('B_Input_x,y,z:',BIx:19,', ',BIy:19,', ',BIz:19,' T');
  Magfeld_Turbo_Berechnen(+0.00,0.01,0.01,1.0);  {drei kartesische Komponenten für Aufpunkt und Strom = 1.0 Ampere}
...
{GG}If i>1 then cr:=Reibung_nachregeln;  {Dieser wird nachgeregelt, um eine konstante Drehzahl einzustellen für stabilen Betrieb der Maschine.}
